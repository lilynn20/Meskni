...
export function AccountPage() {
  ...
  return (
    <main className="site-shell account-shell">
      ...
      <section className="account-panel"><p className="eyebrow">Your account</p><h1>Good to have you, {user?.name}.</h1><p className="welcome-copy">Your Meskni account is ready. Your next step is waiting below.</p><div className="account-actions">{user?.role === 'seeker' && <Link className="button button-dark" to="/saved">Saved listings</Link>}<Link className="button button-quiet" to="/messages">Messages</Link>{user?.role === 'admin' && <Link className="button button-dark" to="/reports">Moderation queue</Link>}{(user?.role === 'owner' || user?.role === 'admin') && <Link className="button button-dark" to="/listings/new">Create a listing</Link>}</div><dl className="account-details"><div><dt>Email</dt><dd>{user?.email}</dd></div><div><dt>City</dt><dd>{user?.city}</dd></div><div><dt>Account type</dt><dd>{user?.role}</dd></div></dl></section>
    </main>
  )
}